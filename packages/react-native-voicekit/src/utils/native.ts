        ...
        return async (...arguments_: unknown[]) => {
          try {
            ...
            const error_ =
              ...
              Object.values(NativeVoiceErrorCode).includes(error.code as NativeVoiceErrorCode)
                ? new VoiceError(
                    'message' in error && typeof error.message === 'string' ? error.message : 'Unknown error',
                    error.code as NativeVoiceErrorCode
                  )
                : new VoiceError('Unknown error', NativeVoiceErrorCode.UNKNOWN, error);
            throw error_;
          }
        };
      }
